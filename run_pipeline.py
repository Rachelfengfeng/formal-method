...
def get_sketch_completion_commands(args, test=False):
    """Completing steps in sketches."""

    model_completion = args.model_completion.split(":")
    output_name = model_completion[1]
    if len(model_completion) == 3:
        if "/" not in model_completion[2]:
            model_completion[2] = f"{args.username}/{model_completion[2]}"
        output_name += "_" + model_completion[2][model_completion[2].find("/")+1:]
    model_name = ":".join(model_completion)

    if args.eval_data:
        name = f"{args.models[-1]}_eval_{shorten_eval_data_name(args.eval_data)}"
        input_dataset = f"{name}"
        output_dataset = f"{name}_ver-{output_name}"
    else:
        input_dataset = f"{args.models[-1]}_infer"
        output_dataset = f"{args.models[-1]}_infer_ver-{output_name}"

    if args.stage == "complete-steps":
        split = "test" if test else "train_base test"
        command = ("python gen_data.py "
                    f"{model_name} {input_dataset} {output_dataset} "
                    f"--complete_steps --splits {split} --num_samples {args.eval_num_samples} "
                    "--output_key formal_proof --input_key formal_statement ")
    else:
        assert not args.eval_data
        command = get_training_command(output_dataset, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("stage", type=str,
                        choices=[
                                ...
                                # Completing steps in sketches
                                "complete-steps", "complete-steps-train",
                                # Preparing test data
    ...
    parser.add_argument("--model_completion", type=str, default="deepseek-ai/DeepSeek-Prover-V2-7B")
    ...
    parser.add_argument("--base_model", type=str, default="Qwen/Qwen2.5-3B-Instruct", help="Base model for fine-tuning.")
    ...
    elif args.stage in ["complete-steps", "complete-steps-train"]:
        commands.append(get_sketch_completion_commands(args, test=args.test))
