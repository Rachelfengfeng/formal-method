...
from utils import save_results_to_dataset_repo, load_results_from_dataset_repo, clear_repo, push_dataset_to_hub
from answer_corruption import add_corrupted_answer_columns
from verify import verify, add_lean_results
from complete_steps import prepare_steps_dataset, aggregate_step_results, verify_and_parse_dataset
from generation import DataThread
...
def process_one_sample(
    ...
    if args.verify:
        if not (need_verification_for_last_sample and sample_idx == start_sample_idx):
            ...
            push_dataset_to_hub(args.output, split, ds_current, commit_message)
    ...
    if args.verify:
        ...
        save_results_to_dataset_repo(current_stats, args.output, "results.jsonl")
...
def generate(input, output, split, args=None):
    ds = load_dataset(input, split=split) if isinstance(input, str) else input
    num_examples = len(ds) if not args.num_examples else min(args.num_examples, len(ds))
    ds = ds.select(range(num_examples))
    step_mapping = {}

    if args.complete_steps:
        assert not args.retry
        ds, ds_ori, num_examples = prepare_steps_dataset(ds, step_mapping, args.num_samples)
    elif args.retry:
    ...
    if "/" in args.model or ".yaml:" in args.model:
        args.njobs = 1
    ...
    if args.complete_steps:
        ds_all_output = aggregate_step_results(ds_all_output, ds_ori, step_mapping, args.num_samples)
    elif args.retry:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--complete_steps", action="store_true", help="For completing steps in the sketch.")
    ...
    if args.complete_steps:
        ...
        ds, updated = verify_and_parse_dataset(ds, args.output_key)
        if updated:
            ds.push_to_hub(args.input, private=True, commit_message="Verify and parse sketches")
